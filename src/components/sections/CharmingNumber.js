...
export default class BeautifyNumCopy extends Component {
  ...
  componentDidMount() {
    // fetch(
    Tabletop.init({
      key: "1LD-4OgAkZACGN34AmaQZe5LLDiOBVHJTvWGHi_Kr1QU",
      simpleSheet: true,
    })
      // )
      // .then((res) => res.json())
      .then((data) => {
        let dataFromApi = data
          .filter((filterBeautiful) => {
            if (filterBeautiful.type === "เลขมงคล") {
              return true;
            } else return false;
          })
          .map((number) => {
    ...
    const dataRender = {
      ...
      rows: [
        ...this.state.users.map((data, i) => ({
          phoneNumber:
          JSON.stringify(data.phoneNumber).replace(/"/g, ""),
          ...
          phoneNumber2:
          JSON.stringify(data.phoneNumber2).replace(/"/g, ""),
